makeF = (e) ->
  ->
    verify(e.time, e.timeExp,
      PLAYER: created_date: '2014-02-14T12:42:04+08:00'
      test:
        date0: '2014-09-12T12:42:00'
        date1: '2014-09-13T12:42:00').should.equal e.result
    return
...
describe 'Time', ->
  ...
  for k of verifyTests
    e = verifyTests[k]
    f = makeF(e)
    it 'Unit Test#' + k, f
  return
describe 'Triger just in time', ->
  t = 
    time:
      time: 'time@Arguments'
      startOf: 'day'
  ...
  check.forEach (plan) ->
    it plan.checkpoint, (done) ->
      plan.idx.forEach (idx) ->
        e = cfg[idx]
        e.check.forEach (ck) ->
          key = 'e' + idx
          ret1 = verify(nowTimeLst[ck[0]], e.time, {})
