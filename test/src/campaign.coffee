Counter = require('../js/counter').Counter
require 'should'
describe 'Campaign', ->
  describe 'Counter', ->
    testConfig = 
      ...
      circleCounterWithCountDown:
        ...
        count_down:
          ...
          units: 'day'
    it 'incr && decr', ->
      counter = new Counter(testConfig.rmb)
      ...
      return
    it 'combo', ->
      counter = new Counter(testConfig.pvp_win)
      ...
      return
    it 'count down', ->
      counter = new Counter(testConfig.check_in)
      counter.incr(1, '2012-12-12').counter.should.equal 1
      counter.incr(1, '2012-12-12').counter.should.equal 1
      counter.incr(1, '2012-12-13').counter.should.equal 2
      ...
      return
    it 'up limit', ->
      counter = new Counter(testConfig.milionaire_goblin)
      counter.incr(1, '2012-12-12').counter.should.equal 1
      counter.incr(1, '2012-12-12').counter.should.equal 2
      ...
      return
    it 'circle add', ->
      counter = new Counter(testConfig.circleCounter)
      counter.incr(1, '2012-12-12').counter.should.equal 1
      counter.incr(1, '2012-12-12').counter.should.equal 2
      counter.incr(1, '2012-12-12').counter.should.equal 3
      counter.incr(1, '2012-12-12').counter.should.equal 1
    ...
    it 'circle decr', ->
      ...
      counter.incr(1, '2012-12-12').counter.should.equal 2
      return
    it 'circle add with countDown', ->
      counter = new Counter(testConfig.circleCounterWithCountDown)
      counter.incr(1, '2012-12-12').counter.should.equal 1
      counter.incr(1, '2012-12-12').counter.should.equal 1
      counter.incr(1, '2012-12-13').counter.should.equal 2
      counter.incr(1, '2012-12-14').counter.should.equal 3
      counter.incr(1, '2012-12-14').counter.should.equal 3
      counter.incr(1, '2012-12-15').counter.should.equal 1
      counter.incr(2, '2012-12-15').counter.should.equal 1
      counter.incr(3, '2012-12-16').counter.should.equal 1
      return
    return
  describe 'Campaign', ->
    Campaign = require('../js/campaign').Campaign
    it 'Goblin', ->
      counter = 
        key: 'goblin'
        initial_value: 0
    ...
    it 'Endless/Hunting', ->
      config = 
        ...
        available_condition: [ {
          type: 'function'
          func: (theData, utils) ->
            Math.floor(utils.libTime.diff(theData.time, '2014-06-14').as('week')) % 2 == 0
        ...
        reset_condition: or: [
          {
            type: 'function'
            func: (theData, utils) ->
              !theData.object.timestamp.infinite

          }
          {
            type: 'time'
            timeExpr: not: [ {
              time: 'infinite@Timestamp'
              units: 'week'
            } ]
          }
        ]
      ...
      campaign.reset player, '2014-06-21'
      player.stage[120].level.should.equal 0
    ...
    it 'Startup', ->
      ...
      configPlayer = 
        ...
        available_condition: [
          ...
          }
          {
            type: 'function'
            func: (theData, utils) ->
              campaignServer.isActive theData.object.getServer(), theData.time

          }
        ]
        activate: (theData, util) ->
          `var server`
          obj = theData.object
          server = obj.getServer()
          prize = server.startup_reward
          obj.mail = prize
          return
      configServer = 
        storeType: 'server'
        ...
        update: (theData, util) ->
          obj = theData.object
          counter = obj.counters.startupReward
          obj.startup_reward = counter.counter
          return
      campaignPlayer = new Campaign(configPlayer)
      campaignServer = new Campaign(configServer)
      ...
      campaignServer.activate server, 1, '2014-06-14'
      campaignServer.update server
      server.startup_reward.should.equal 0
      campaignPlayer.activate player, 1, '2014-06-14'
      player.mail.should.equal 0
      campaignServer.activate server, 1, '2014-06-14'
      campaignServer.update server
      server.startup_reward.should.equal 0
      campaignServer.activate server, 1, '2014-06-14'
      ...
      campaignServer.activate server, 1, '2014-06-15'
      campaignServer.update server
      campaignPlayer.activate player, 1, '2014-06-15'
      player.mail.should.equal 1
      server.startup_reward.should.equal 1
      campaignPlayer.activate player, 1, '2014-06-15'
      player.mail.should.equal 1
      campaignServer.activate server, 1, '2014-06-16'
      campaignServer.update server
